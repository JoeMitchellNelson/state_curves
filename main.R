...
# going to calculate 3-day moving averages. 
...
#  geom_hline(yintercept=0,size=.5,color="grey90") +
  ...
  labs(x="",
      # y=paste0("Deaths per 100,000 per day\n(",ma_length,"-day moving average)\n")) +
       y="") +
  lims(y=c(0,max(nyt[which(nyt$state==state_name),]$ma_pc_death))) +
  theme(axis.title = element_text(family="Book Antiqua"),
        axis.text = element_text(family="Book Antiqua"),
...
ggsave("~/state_curves/top.png",last_plot(),width=44,height=16,units="in")
...
ggsave("~/state_curves/bottom.png",last_plot(),width=44,height=16,units="in")
...
top / bottom + plot_annotation(title=paste0("Daily Covid-19 deaths per 100,000 people in each state\n(",ma_length,"-day moving average)\n"),
                               caption="Source: New York Times and US Census Bureau        \nMeasures of Covid-19 deaths subject to testing capacity, which may vary by state        \nNote that the y-axis scale differs for each state        \n",
                               theme=theme(plot.title = element_text(family="Book Antiqua",size=50,hjust=.5,vjust=.1),
                                           plot.caption = element_text(family="Book Antiqua",size=35))) 

ggsave("~/state_curves/big.png",last_plot(),width=44,height=35,units="in")
